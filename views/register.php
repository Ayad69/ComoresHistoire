...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dateNaissUser = $_POST['dateNaissUser'] ?? '';
    $password = $_POST['password'] ?? '';
    $password_confirm = $_POST['password_confirm'] ?? '';
    $ecrivain = isset($_POST['ecrivain']) && $_POST['ecrivain'] === '1'; // false par défaut

    if (strlen($nomUser) < 1 || strlen($nomUser) > 50) $errors[] = 'Le nom est requis (max 50 caractères).';
    if (strlen($prenomUser) < 1 || strlen($prenomUser) > 50) $errors[] = 'Le prénom est requis (max 50 caractères).';
    if (!filter_var($emailUser, FILTER_VALIDATE_EMAIL)) $errors[] = 'Email invalide.';
    if (strlen($emailUser) > 50) $errors[] = 'Email trop long.';
    if (strlen($password) < 6) $errors[] = 'Le mot de passe doit contenir au moins 6 caractères.';
    if ($password !== $password_confirm) $errors[] = 'Les deux mots de passe ne correspondent pas.';
    ...
    if (empty($errors)) {
        try {
            ...
            if ($userDao->readByEmail($emailUser) !== null) {
                ...
                $user->setDateNaissUser($dateNaissUser !== '' ? \DateTime::createFromFormat('Y-m-d', $dateNaissUser) : null);
                $user->setPassword(password_hash($password, PASSWORD_DEFAULT));
                $user->setEcrivain($ecrivain); // false si case non cochée
                $userDao->create($user);
...
<head>
    ...
    <title>Inscription — Histoire des Comores</title>
...
<main class="page-auth">
    <div class="auth-card">
        <h1>Inscription</h1>
        <p class="auth-intro">Créez votre compte pour participer à l'histoire des Comores.</p>
        ...
        <?php if (is_readable(__DIR__ . '/../config/google_client_secret.json')): ?>
            <a href="google_redirect.php" class="btn-google" title="S'inscrire avec Google">
        ...
        <form action="" method="post" class="form-auth">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="nomUser" name="nomUser" maxlength="50" required
                           placeholder="Votre nom" autocomplete="family-name">
                </div>
                <div class="form-group">
                    <label for="prenomUser">Prénom</label>
                    <input type="text" id="prenomUser" name="prenomUser" maxlength="50" required
            ...
            <div class="form-group">
                <label for="dateNaissUser">Date de naissance</label>
                <input type="date" id="dateNaissUser" name="dateNaissUser" autocomplete="bday">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="password" id="password_confirm" name="password_confirm" maxlength="255" required
                       placeholder="••••••••" autocomplete="new-password">
            </div>
